###############################################################################
#
# Opens chosen URL in Chromium in kiosk mode.
# PRODUCTION READY!!!
#
###############################################################################
...
def is_chromium_running():
    try:
        ...
        result = subprocess.run(['pgrep', '-f', 'chromium'], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
def open_chromium(url):
    ...
    env = os.environ.copy()
    env["DISPLAY"] = ":1"  # Target the Raspberry Pi's screen
